...
// Todas as tabs — banco só aparece para o dono (filtrado abaixo)
const TABS_BASE = [
  ...
  { id: 'banco',        label: 'Banco',           icon: Database, ownerOnly: true },
...
export default function App() {
  ...
  // Tabs filtradas: banco só para o dono
  const TABS = TABS_BASE.filter(t => !t.ownerOnly || isOwner);

  // Se não for dono e estiver na aba banco, redireciona
  useEffect(() => {
    if (!isOwner && tab === 'banco') setTab('dashboard');
  }, [isOwner, tab]);
  ...
  const renderPage = () => {
    switch (tab) {
      ...
      case 'banco':        return isOwner ? <BancoQuestoes /> : <Dashboard onNavigate={setTab} />;
